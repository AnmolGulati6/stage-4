...
HeapFile::HeapFile(const string & fileName, Status& returnStatus)
{
    ...
    {   
        ...
        if(status != OK) {
            cerr << "open of heap file failed at getFirstPage\n";
            returnStatus = status;
        } 
        ...
        if(status != OK) {
            cerr << "open of heap file failed at read(first)Page\n";
            returnStatus = status;
        }
        ...
        if(status != OK) {
            cerr << "open of heap file failed at readPage\n";
            returnStatus = status;
        }
...
const Status HeapFile::getRecord(const RID & rid, Record & rec)
{
	...
	if(curPage == NULL) {
		statusOne = bufMgr->readPage(filePtr, rid.pageNo, curPage);
		if(statusOne != OK) return statusOne;
		curDirtyFlag = false;
...
// scans pages in heapfile for record fitting the matchRec filter
